...
def with_pagination(base_url: str, limit: int, offset: int) -> str:
...
def fetch_page(base_url: str, limit: int, offset: int, timeout: int) -> tuple[str, List[Dict[str, Any]]]:
    url = with_pagination(base_url, limit, offset)
    ...
    if isinstance(payload, dict):
        ...
        return url, (events if isinstance(events, list) else [])
    return url, []
...
def run_once(tag_slug: str) -> int:
    ...
    while True:
        request_url, events = fetch_page(base_url, LIMIT, offset, TIMEOUT_SECONDS)
